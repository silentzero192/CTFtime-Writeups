...
3. Action: Calculated `0.01s` vs `0.10s` gaps from `if2_100k.tsv`, mapped them to bits and appended a leading zero. Result: ASCII conversion produced the flag.
...
6) Timing differences can encode entire messages when headers/payloads stay constant; check inter-arrival stats whenever traffic looks uniform.
